...
กฎนี้อนุญาตเฉพาะบัญชี Google ให้แก้โปรไฟล์ ความคืบหน้า ตำแหน่ง และข้อมูลสมาชิก Raid ของ UID ตนเอง บัญชี Anonymous ถูกปฏิเสธ ผู้ที่ไม่ได้อยู่ในห้องอ่านข้อมูล Raid ไม่ได้ การโจมตีหนึ่งครั้งถูกจำกัดไม่เกิน 40 HP และอีโมตใช้ได้เฉพาะรายการที่ระบบกำหนด อีเมลไม่ถูกเก็บใน Realtime Database และไม่แสดงแก่ผู้เล่นอื่น ตำแหน่งออนไลน์รองรับครบ 4 โลก ได้แก่ ศูนย์กลาง ป่าฝึก นครกฎหมาย และมหานครอนาคต
...
หากขึ้น `Firebase ปฏิเสธสิทธิ์` ให้เปิด **บัญชีและตัวละคร → ตรวจสิทธิ์ Firebase** ระบบจะทดสอบแยก 4 จุด: โปรไฟล์, Cloud Save, สถานะออนไลน์ และการสร้าง/ลบห้อง Raid ชั่วคราว โดยไม่แสดงอีเมลหรือรหัสยืนยันตัวตน

- ถ้าทุกข้อไม่ผ่าน: ออกจากระบบ แล้วเข้าสู่ระบบ Google ใหม่ จากนั้นตรวจ Authorized domains และ Rules
- ถ้า 3 ข้อแรกผ่าน แต่ **สร้างและลบห้อง Raid ทดสอบ** ไม่ผ่าน: Realtime Database ยังใช้ Rules ชุดเก่า ให้คัดลอก `database.rules.json` เวอร์ชันล่าสุดทั้งไฟล์และกด **Publish**
- Rules รุ่น `2026-07-18.4` รองรับโซนออนไลน์ของโลกกฎหมายและโลกอนาคต พร้อมคงการตรวจ `bossHp` เทียบกับ `bossMax` ของข้อมูลหลังเขียน (`newData`)
- ถ้าผ่านทั้ง 4 จุด: ปิดหน้าต่างตรวจแล้วสร้าง Raid จริงได้ทันที ไม่ต้อง Publish ซ้ำ
...
9. ทดสอบหน้าจอมือถือ: ปุ่มตอบและโจมตีต้องกดง่าย ไม่มีเนื้อหาล้นแนวนอน และในโลกผจญภัยสามารถแตะพื้นเพื่อสั่งตัวละครเดินได้
