...
กฎนี้อนุญาตเฉพาะบัญชี Google ให้แก้โปรไฟล์ ความคืบหน้า ตำแหน่ง ข้อความใกล้ตัว สัญญาณจับคู่ Voice และข้อมูลสมาชิก Raid ของ UID ตนเอง บัญชี Anonymous ถูกปฏิเสธ ผู้ที่ไม่ได้อยู่ในห้องอ่านข้อมูล Raid ไม่ได้ การโจมตีหนึ่งครั้งถูกจำกัดไม่เกิน 40 HP และอีโมตใช้ได้เฉพาะรายการที่ระบบกำหนด อีเมลไม่ถูกเก็บใน Realtime Database และไม่แสดงแก่ผู้เล่นอื่น ตำแหน่งออนไลน์รองรับครบ 4 โลก ได้แก่ ศูนย์กลาง ป่าฝึก นครกฎหมาย และมหานครอนาคต โดยใช้หนึ่งห้อง Presence ต่อหนึ่งแผนที่จริง ผู้เล่นในโซนย่อยต่างกันของแผนที่ศูนย์กลางจึงยังเห็นกันได้ ข้อความจำกัด 80 ตัวอักษรและเก็บล่าสุดเพียงหนึ่งข้อความต่อผู้เล่นต่อแผนที่

### Voice Chat ตามระยะ

- ผู้เล่นต้องกดเปิด Voice และอนุญาตไมโครโฟนด้วยตนเอง ไมค์เริ่มต้นเป็นปิด
- กดค้างปุ่ม Push-to-talk หรือปุ่ม `V` เพื่อส่งเสียง ปล่อยแล้วไมค์ปิดทันที
- เสียงวิ่งตรงระหว่างเบราว์เซอร์ด้วย WebRTC และไม่ได้ถูกบันทึกลง Firebase
- Firebase เก็บเฉพาะ SDP สำหรับจับคู่ชั่วคราวใน `voiceSignals` ผู้ส่งเขียนได้เฉพาะช่องของตนและผู้รับอ่านได้เฉพาะ inbox ของตน
- เชื่อมเสียงสูงสุด 4 คนในระยะ 360 หน่วย ปิดเสียงรายคนหรือปิด Voice ทั้งระบบได้ตลอดเวลา
- ใช้งานไมโครโฟนได้บน HTTPS เท่านั้น และบางเครือข่ายมือถือ/NAT อาจเชื่อม P2P ไม่สำเร็จหากไม่มี TURN server

### เพิ่มบัญชี Test Admin

1. เปิด Realtime Database แท็บ **Data**
2. เพิ่มโหนด `admins`
3. ใต้ `admins` เพิ่มชื่อโหนดเป็น Firebase Authentication **User UID** ของบัญชีทดสอบ
4. ตั้งค่าเป็น Boolean `true`
5. ออกจากระบบเว็บแล้วเข้าสู่ระบบ Google ใหม่ เพื่อให้เว็บตรวจ Role อีกครั้ง

ไคลเอนต์อ่านได้เฉพาะ `admins/{UID ของตนเอง}` และเขียน Role ไม่ได้ การเพิ่มหรือลบ Admin ต้องทำจาก Firebase Console ของเจ้าของโปรเจกต์เท่านั้น แผง Test Admin เพิ่ม Coin/EXP ปลดล็อกไอเทม เปิดหมอก และวาร์ปเพื่อทดสอบ แต่ไม่สามารถอ่านหรือแก้ข้อมูลส่วนตัวของผู้เล่นอื่น
...
หากขึ้น `Firebase ปฏิเสธสิทธิ์` ให้เปิด **บัญชีและตัวละคร → ตรวจสิทธิ์ Firebase** ระบบจะทดสอบแยก 7 จุด: โปรไฟล์, Cloud Save, สถานะออนไลน์, การอ่าน World Presence, การส่ง/ลบตำแหน่งตัวละคร, แชตใกล้ตัว และการสร้าง/ลบห้อง Raid ชั่วคราว โดยไม่แสดงอีเมลหรือรหัสยืนยันตัวตน

- ถ้าทุกข้อไม่ผ่าน: ออกจากระบบ แล้วเข้าสู่ระบบ Google ใหม่ จากนั้นตรวจ Authorized domains และ Rules
- ถ้า **อ่านผู้เล่นในพื้นที่** หรือ **ส่งและลบตำแหน่งตัวละครทดสอบ** ไม่ผ่าน: ให้คัดลอก `database.rules.json` เวอร์ชันล่าสุดทั้งไฟล์และกด **Publish**
- ถ้าทุกข้อผ่านยกเว้น **สร้างและลบห้อง Raid ทดสอบ**: Realtime Database ยังใช้ Rules ชุดเก่า ให้คัดลอกไฟล์ล่าสุดและกด **Publish** เช่นกัน
- Rules รุ่น `2026-07-22.1` ตรวจ World Presence แยกอ่าน/เขียน และใช้หนึ่งห้อง Presence ต่อหนึ่งแผนที่จริง โดยยังคงแผนที่ทั้ง 4 โลก
- ถ้าผ่านทั้ง 7 จุด: ปิดหน้าต่างตรวจแล้วทดสอบผู้เล่นสองบัญชีได้ทันที ไม่ต้อง Publish ซ้ำ
...
4. เปิดเว็บด้วย Google คนละบัญชีในสองเบราว์เซอร์ ตั้งชื่อและชุดคนละแบบ แล้วให้ตัวละครอยู่แผนที่เดียวกัน (ในแผนที่ศูนย์กลางจะอยู่คนละโซนย่อยก็ได้)
